      ...
      {requiredPrayers.map((prayer) => {
        ...
        return (
          <motion.div
            ...
            key={prayer}
            className={`
              p-4 rounded-2xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 transition-all border
          ...
          >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto flex-row">
              {isExcused ? (
                <div className="w-full h-11 sm:w-auto px-4 rounded-full sm:rounded-full rounded-2xl bg-sky-500/10 border-2 border-sky-500/30 text-sky-600 dark:text-sky-400 flex items-center justify-center" aria-hidden="true">
                  <span className="text-xs font-bold leading-none">EXCUSED</span>
                </div>
              ) : (
                <>
                  <button
                    ...
                    className={`h-11 min-w-[44px] rounded-full border-2 flex w-full sm:w-auto items-center justify-center gap-1.5 overflow-hidden transition-all active:scale-90
                      ${isMissed ? 'px-4 bg-amber-500/10 border-amber-500/40 text-amber-600 dark:text-amber-500' : 'px-3 sm:px-3 border-border hover:border-amber-500/40 hover:bg-amber-500/5 text-muted-foreground hover:text-amber-600'}
                  ...
                  <button
                    ...
                    className={`h-11 min-w-[44px] rounded-full border-2 flex w-full sm:w-auto items-center justify-center gap-1.5 overflow-hidden transition-all active:scale-90
                      ${isDone ? 'px-4 bg-primary border-primary text-primary-foreground' : 'px-3 sm:px-4 border-border hover:border-primary/40 hover:bg-primary/5 text-muted-foreground hover:text-primary'}
                      ${isFuture || isExcused ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}
                    `}
                  >
