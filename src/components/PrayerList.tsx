      ...
      {requiredPrayers.map((prayer) => {
        ...
        return (
          <motion.div
            ...
            key={prayer}
            className={`
              p-4 rounded-2xl flex flex-col sm:flex-row flex-wrap items-start sm:items-center justify-between gap-3 sm:gap-4 transition-all border
          ...
          >
            ...
            <div className="flex flex-col-reverse sm:flex-row w-full sm:w-auto items-stretch sm:items-center gap-2">
              {isExcused ? (
                <div className="w-full sm:w-11 h-11 rounded-xl sm:rounded-full bg-sky-500/10 border-2 border-sky-500/30 text-sky-600 dark:text-sky-400 flex items-center justify-center" aria-hidden="true">
                  <span className="text-xs sm:text-[10px] font-bold leading-none sm:hidden">EXCUSED</span>
                  <span className="hidden sm:inline text-[10px] font-bold leading-none">EX</span>
                </div>
              ) : (
                <>
                  <button
                    ...
                    className={`h-11 min-w-[44px] rounded-xl sm:rounded-full border-2 flex items-center justify-center gap-1.5 overflow-hidden transition-all active:scale-90
                      ${isMissed ? 'w-full sm:w-auto px-4 bg-amber-500/10 border-amber-500/40 text-amber-600 dark:text-amber-500' : 'w-full sm:w-auto px-4 sm:px-3 border-border hover:border-amber-500/40 hover:bg-amber-500/5 text-muted-foreground hover:text-amber-600'}
                  ...
                  <button
                    ...
                    className={`h-11 min-w-[44px] rounded-xl sm:rounded-full border-2 flex items-center justify-center gap-1.5 overflow-hidden transition-all active:scale-90
                      ${isDone ? 'w-full sm:w-auto px-4 bg-primary border-primary text-primary-foreground' : 'w-full sm:w-auto px-4 sm:px-4 border-border hover:border-primary/40 hover:bg-primary/5 text-muted-foreground hover:text-primary'}
                      ${isFuture || isExcused ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}
                    `}
                  >
