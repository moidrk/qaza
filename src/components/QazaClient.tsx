          ...
          {Object.entries(stats.backlog).map(([prayer, count]) => {
            ...
            return (
              <Card 
                ...
                className={`!flex-row items-center justify-between transition-all overflow-hidden cursor-pointer active:scale-[0.98] p-4 min-h-[80px] shadow-sm w-full ${
                  isCaughtUp
              ...
              >
                <div className="flex flex-col items-start text-left">
                  <h3 className={`text-lg font-bold ${isCaughtUp ? 'text-muted-foreground' : 'text-foreground'}`}>{prayer}</h3>
                  {isCaughtUp ? (
                ...
                <ChevronRight className="text-muted-foreground opacity-50" size={24} aria-hidden="true" />
              </Card>
            )
          })}
        </div>
