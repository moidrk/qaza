...
2024-05-24 - [Stacked Actions for Mobile]
Learning: In mobile view, side-by-side action buttons inside prayer cards can result in cramped touch targets that are hard for thumb use.
Action: Use stacked flex layouts (`flex-col-reverse sm:flex-row` with `w-full sm:w-auto`) to position primary actions prominently with maximum width on mobile without regressing horizontal desktop views.
